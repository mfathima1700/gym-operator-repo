...
// Type definitions for sort options and filters
type SortOption = {
  name: string;
  href: string;
};
        ...
        <section aria-labelledby="filter-heading" className="border-t border-gray-200 py-6">
          ...
          <div className="flex items-center justify-between">
                ...
                <div className="py-1">
                  {sortOptions.map((option) => (
                    <MenuItem key={option.name}>
                      <a
                        href={option.href}
                        className="block px-4 py-2 text-sm font-medium text-white data-focus:bg-gray-100 data-focus:outline-hidden"
                      >
                        {option.name}
                      </a>
                    </MenuItem>
            ...
            <button
              ...
              className="inline-block text-sm font-medium text-gray-300 hover:text-white sm:hidden"
            >
              Filters
            </button>
            <PopoverGroup className="hidden sm:flex sm:items-baseline sm:space-x-8">
              {filters.map((section, sectionIdx) => (
                <Popover key={section.name} id={`desktop-menu-${sectionIdx}`} className="relative inline-block text-left">
                  <div>
                    <PopoverButton className="group inline-flex items-center justify-center text-sm font-medium text-gray-300 hover:text-gray-900">
